...
library(party)
fraudTotal.db_cat <- subset(fraudTotal.db, select = c(4, 5, 7, 23))
fraudTotal.db_cat$is_fraud <- as.factor(fraudTotal.db_cat$is_fraud)

fraudTotal.db_cat_RF <- cforest(is_fraud~., data = fraudTotal.db_cat, control = cforest_unbiased(mtry = 2, ntree = 50))
varimp(fraudTotal.db_cat_RF)
